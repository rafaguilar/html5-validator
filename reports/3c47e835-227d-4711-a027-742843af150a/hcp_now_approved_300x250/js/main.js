var _loadedImages = 0,
	_imageArray = new Array('bgf1.png','bgf3.png','bgf5.png','bgf6.png','cta_arrow.png','cta.png','f1txt1.png','f1txt2.png','f2txt1.png','f3txt1.png','f4txt1.png','f5txt1.png','logo.png'),
	_tl,
...
function initAnimations() {
	...
	function FRAME_1() {
		gsap.to('.banner', { duration: .2,  opacity:1 });
		gsap.to('#f1txt1', { duration: .5,  x: 46, opacity: 1, ease:'power1.out' });

		gsap.delayedCall( 2.5, FRAME_2)
	}

	function FRAME_2() {
		gsap.to(['#f1txt1', '#f1txt2', '#bgf1', '#bgf2'], { duration: .5,  x: -300, ease:'power1.out'/*, delay: 1.5*/ });
		gsap.to('#f2txt1', { duration: .6, x: -280, ease:'power1.out', delay: .2 });

		gsap.delayedCall( 2.8, FRAME_3)
	}

	function FRAME_3() {
		gsap.to(['#f2txt1', '#bgf2'], { duration: .4,  opacity: 0 });
		gsap.to('#bgf3', { duration: .5, y: 152, ease:'power1.out' });

		gsap.delayedCall( 3, FRAME_4)
	}

	function FRAME_4() {
		gsap.to('#bgf3', { duration: .4, y: 265, ease:'power1.out' });
		gsap.to('#f3txt1', { duration: .3, opacity: 0 });
		gsap.to('#f4txt1', { duration: .5, opacity: 1, delay: .4 });
		gsap.to('#bgf5', { duration: .5, y: 20, ease:'power1.out', delay: .4 });

		gsap.delayedCall( 3.2, FRAME_5)
	}

	function FRAME_5() {
		gsap.to('#bgf5', { duration: .6, y: 331, ease:'power1.out' });
		gsap.to('#f4txt1', { duration: .6, opacity: 0, delay: .2 });
		gsap.to('#bg_cta', { duration: .5, y: -85, ease:'power1.out', delay: 1 });
		gsap.to(['#cta','#cta_arrow'], { duration: .6, opacity: 1, delay: 1.6 });

		gsap.delayedCall( 2.2, overButton)

		_tl
		.addLabel('autoISI')
		.to(_isiText, { duration: _isiFullTime, scrollTo: { y: _isiFullHeight }, ease: 'none' },'+=2.6');
		_tl.seek('autoISI');
	}
}
...
function overButton() {
	elem('#cta_btn').addEventListener('mouseover', function () {
		gsap.to('#cta_arrow', { duration: .25, x: 0 + 3, ease:'power1.out' });
	});
